...
function positionTextItem(item: TextItemLike, viewport: PdfViewportLike): PositionedTextItem {
  ...
  // PDF.js normally reports vertical Japanese as `ttb`, but some generated PDFs
  // only expose the rotation in their text matrix. Accept either signal.
  const verticalByMatrix = Math.abs(transformed[1]) > Math.abs(transformed[0]) * 0.75;
  const vertical = item.dir === 'ttb' || verticalByMatrix;
  const x = vertical ? transformed[4] : transformed[4] + Math.abs(item.width) / 2;
...
function clusterColumns(items: PositionedTextItem[], tolerance: number): TextColumn[] {
  ...
  for (const item of [...items].sort((a, b) => b.x - a.x)) {
    ...
    if (!column) {
      column = { x: item.x, items: [], yMin: item.y, yMax: item.y };
      columns.push(column);
    }
...
function nearestColumn(item: PositionedTextItem, columns: TextColumn[], maxDistance: number): TextColumn | null {
  return (
    columns
...
function parseHeading(text: string): PageHeading | null {
  ...
  if (compact.endsWith(PREFACE_SUFFIX)) {
    return { title: compact.slice(0, -PREFACE_SUFFIX.length), kind: 'preface' };
  }

  if (compact.endsWith(AFTERWORD_SUFFIX)) {
    return { title: compact.slice(0, -AFTERWORD_SUFFIX.length), kind: 'afterword' };
  }
...
function bodyFontSizeFor(items: PositionedTextItem[]): number {
  const sizes = items
    .filter((item) => item.vertical && item.text.trim())
    .map((item) => item.fontSize)
    .filter((size) => Number.isFinite(size) && size >= 10 && size <= 24);
  return median(sizes);
}

function buildColumns(positioned: PositionedTextItem[], viewport: PdfViewportLike): { columns: TextColumn[]; bodyFontSize: number } {
  const interior = positioned.filter(
    (item) => item.y >= viewport.height * 0.06 && item.y <= viewport.height * 0.94,
  );
  ...
  const mainAnchors = interior.filter(
    (item) => item.vertical && item.text.trim() && item.fontSize >= rubyThreshold,
  );
  ...
  for (const item of interior) {
    ...
    if (item.vertical && item.text.trim() && item.fontSize < rubyThreshold) {
      rubyItems.push(item);
      continue;
    }
    ...
    if (!item.vertical) {
      ...
      if (!compact) continue;
      if (item.y >= viewport.height * 0.84 && /^[0-9０-９]+$/.test(compact)) continue;
    }
...
function detectPageHeading(columns: TextColumn[], bodyFontSize: number, viewport: PdfViewportLike): HeadingDetection {
  ...
  if (!rightmost || !second) {
    return { heading: null, headingColumn: null, skipContent: false, candidate: null };
  }

  const text = compactText(columnText(rightmost));

  // Narou's landscape vertical PDF places episode headings in a dedicated far-right
  // column. Its exact x coordinate can differ slightly depending on PDF.js/font metrics,
  // so identify the layout instead of comparing against one magic x value.
  const inRightHeadingBand =
    rightmost.x >= viewport.width * 0.80 && rightmost.x <= viewport.width * 0.94;
  const startsInHeadingBand =
    rightmost.yMin >= viewport.height * 0.165 && rightmost.yMin <= viewport.height * 0.24;
  const separatedFromBody = rightmost.x - second.x >= bodyFontSize * 4.0;
  const plausibleLength = text.length > 0 && text.length <= 100;

  if (!inRightHeadingBand || !startsInHeadingBand || !separatedFromBody || !plausibleLength) {
    return { heading: null, headingColumn: null, skipContent: false, candidate: null };
  }

  if (text === NOTICE_HEADING) {
    return { heading: null, headingColumn: rightmost, skipContent: true, candidate: text };
  }
...
async function extractPage(pdf: NarouPdfDocumentLike, pageNumber: number): Promise<PageExtraction> {
  ...
  try {
    ...
    const { heading, headingColumn, skipContent, candidate } = detectPageHeading(
      columns,
      bodyFontSize,
      viewport,
    );
    ...
    if (skipContent) {
      return { heading: null, segments: [], headingCandidate: candidate ?? undefined };
    }
    ...
    for (const column of bodyColumns) {
      ...
      const startsAfterGap = previousX !== null && previousX - column.x > bodyFontSize * 2.2;

      segments.push({ text, startsParagraph: startsByIndent || startsAfterGap });
      previousX = column.x;
    }

    return { heading, segments, headingCandidate: candidate ?? undefined };
  } finally {
    page.cleanup?.();
  }
}

function appendSegments(paragraphs: string[], segments: ParagraphSegment[], forceFirstParagraph: boolean): void {
  let first = true;
  ...
  for (const segment of segments) {
    ...
    if (startsParagraph) paragraphs.push(text);
...
export async function looksLikeNarouPdf(pdf: NarouPdfDocumentLike, fileName: string): Promise<boolean> {
  const fileBase = fileName.replace(/\.pdf$/i, '').trim();
...
export async function parseNarouPdf(
  ...
  const flushChapter = () => {
    ...
    if (paragraphs.length > 0) {
      chapters.push({ id: `chapter-${chapters.length + 1}`, title: currentTitle, paragraphs });
    }
    currentTitle = null;
    currentParagraphs = [];
    activeSection = 'none';
  ...
  const consumeExtraction = (extraction: PageExtraction) => {
    if (extraction.headingCandidate && extraction.headingCandidate !== NOTICE_HEADING) {
      detectedCandidates.push(extraction.headingCandidate);
    }
  ...
  for (let batchStart = 2; batchStart <= pdf.numPages; batchStart += EXTRACTION_CONCURRENCY) {
    ...
    const extractions = await Promise.all(pageNumbers.map((pageNumber) => extractPage(pdf, pageNumber)));
    for (const extraction of extractions) consumeExtraction(extraction);

    onProgress?.(pageNumbers[pageNumbers.length - 1], pdf.numPages);
  ...
  if (chapters.length === 0) {
    const diagnostic = detectedCandidates.slice(0, 3).join(' / ');
    throw new Error(
      diagnostic
        ? `小説家になろうのPDFとして話を構築できませんでした。見出し候補: ${diagnostic}`
        : '小説家になろうのPDFとして話タイトルを検出できませんでした（右端の見出し列を検出できません）。',
    );
  }
